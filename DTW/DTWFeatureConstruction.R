...
#     2.4. for each point on test dataframe, dtw distance is calculated from the train data frame
#     2.5. min dtw distance is obtained and saved
#     2.6. dtw feature set is constructed using task label obtained from step 1 and dtw min distance obtained from previous step.
#
library(dplyr)  
library(dtw)
DTWFeature.OneParticipant<-function(test1,ppt1)
{
  dtw.df <- data.frame(matrix(nrow = 0, ncol = 2))
  colnames(dtw.df) <- c("Label","Dist")
  ...
  for(i in 1:length(activities))
  {
    ...
    dtw.df <- rbind(dtw.df,intermediate_feature)
  }
  dtw.df
}

save(dtw.df, file = "/home/sumi/Documents/Research_Project/Participant_Data/R_DataFiles/dtwfeatures.RData")